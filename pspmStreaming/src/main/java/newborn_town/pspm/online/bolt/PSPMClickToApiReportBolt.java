 ...
 *
 */
public class PSPMClickToApiReportBolt extends BaseBasicBolt {

	private static final long serialVersionUID = -6707182554696236571L;

	static Logger logger = Logger.getLogger(PSPMClickToApiReportBolt.class);
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		if (logNumber.get(day) == null) {
			...
			for (Map.Entry<String, Integer> entry : logNumber.entrySet()) {
				logger.info("PSPMClickToApiReportBolt" + entry.getKey()
	...
	private void insterOrUpdateDB() {
		...
		logger.info("PSPMClickToApiReportBolt 插入或更新" + updateIncDocs.size()
				+ "条数据！");
