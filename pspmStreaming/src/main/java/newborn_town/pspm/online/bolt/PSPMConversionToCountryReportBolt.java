 ...
 *
 */
public class PSPMConversionToCountryReportBolt extends BaseBasicBolt {

	private static final long serialVersionUID = -3741400208496719631L;

	static Logger logger = Logger
			.getLogger(PSPMConversionToCountryReportBolt.class);
	...
	public void prepare(@SuppressWarnings("rawtypes") Map stormConf,
		...
		mongoUtil = new MongoUtil("pspm", "countryReport");
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		String mogoDBMapKey = PublicUtill.MD5(day + geo + advertiser_id
				+ publisher_id); // 拼接map中的key

		if (logNumber.get(day) == null) {

			for (Map.Entry<String, Integer> entry : logNumber.entrySet()) {
				logger.info("PSPMConversionToCountryReportBolt"
		...
		documentStr.append("_id", mogoDBMapKey).append("country", geo)
				.append("day", day)
	...
	private void insterOrUpdateDB() {
		...
		logger.info("PSPMConversionToCountryReportBolt 插入或更新"
