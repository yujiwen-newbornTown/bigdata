 ...
 *
 */
public class PSPMClickToCampaignReportBolt extends BaseBasicBolt {

	private static final long serialVersionUID = -6707182554696236571L;

	static Logger logger = Logger
			.getLogger(PSPMClickToCampaignReportBolt.class);
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		if (logNumber.get(day) == null) {
			...
			for (Map.Entry<String, Integer> entry : logNumber.entrySet()) {
				logger.info("PSPMClickToCampaignReportBolt" + entry.getKey()
		...
		String mogoDBMapKey = PublicUtill.MD5(day + campaign_id); // 拼接map中的key
	...
	private void insterOrUpdateDB() {
		...
		logger.info("PSPMClickToCampaignReportBolt 插入或更新"
